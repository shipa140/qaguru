...
test.describe("UI: статьи", { tag: ["@ui", "@articles"] }, () => {
  test(
    "Сохранение содержимого опубликованной статьи",
    { tag: "@positive" },
  ...
  test(
    "Сохранение изменений статьи после редактирования",
    { tag: "@positive" },
    async ({ setupApi, ui, resources, userBuilder, articleBuilder }) => {
      const user = userBuilder().build();
      const article = articleBuilder().build();
  ...
  test(
    "Изменение счётчика при добавлении статьи в избранное и удалении из него",
    { tag: "@positive" },
    async ({ setupApi, ui, resources, userBuilder, articleBuilder }) => {
      ...
      const result = await ui.article.favoriteAndUnfavorite(slug, initialCount);
      const persisted = await setupApi.articles.get(slug);
  ...
  test(
    "Предложение войти при попытке гостя добавить комментарий",
  ...
  test(
    "Недоступность статьи после удаления",
    { tag: "@positive" },
    async ({ setupApi, ui, resources, userBuilder, articleBuilder }) => {
      const user = userBuilder().build();
      const article = articleBuilder().build();
