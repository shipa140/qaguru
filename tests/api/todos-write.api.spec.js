...
test.describe("API: изменение задач", { tag: ["@api", "@todos"] }, () => {
  test(
  ...
  test(
    "Сохранение остальных полей при частичном изменении задачи",
    { tag: ["@post", "@positive"] },
    async ({ app, challenger, todoBuilder }) => {
      const todo = todoBuilder().withDoneStatus(true).build();
      const created = await app.todos.create(challenger, todo);
  ...
  test(
    "Сохранение всех полей после полной замены задачи",
    { tag: ["@put", "@positive"] },
    async ({ app, challenger, todoBuilder }) => {
      const created = await app.todos.create(challenger, todoBuilder().build());
  ...
  test(
    "Недоступность задачи после удаления",
    { tag: ["@delete", "@positive"] },
    async ({ app, challenger, todoBuilder }) => {
      const created = await app.todos.create(challenger, todoBuilder().build());
  ...
  test(
    "Создание задачи с телом и ответом в формате XML",
